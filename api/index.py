    ...
    try:
        ...
        if top_fav_docs:
            ...
            for cat in categories_collection.find(
                {'_id': {
                    '$in': [ObjectId(cid) for cid in cat_ids]
                }}):
        ...
        if top_pin_docs:
            ...
            if top_cat:
                ...
                cat_content_ids = [
                    c['_id'] for c in content_collection.find(
                        {'category_id': top_cat_id}, {'_id': 1})
                ]
                top_cat[
...
def check_favorites_bulk():
    ...
    # Filter out synthetic batch IDs (format: "batchid___index") — they share the
    # batch document's real ObjectId so map them back before querying.
    real_id_map = {}  # real_object_id_str -> [original_id_str, ...]
    for raw_id in ids:
        if '___' in str(raw_id):
            real_str = str(raw_id).split('___')[0]
        else:
            real_str = str(raw_id)
        real_id_map.setdefault(real_str, []).append(str(raw_id))

    try:
        object_ids = [ObjectId(k) for k in real_id_map]
    except Exception:
        return jsonify({'favorited': []})
    ...
    result = []
    for doc in favorited_docs:
        real_str = str(doc['content_id'])
        # Return all original IDs that map to this real document
        result.extend(real_id_map.get(real_str, [real_str]))

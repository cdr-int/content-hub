...
@app.route('/api/admin/seed-web-analytics', methods=['POST'])
@admin_required
def seed_web_analytics():
    """Seed Web Analytics documentation into the database"""
    try:
        # Check if category already exists
        ...
        if existing_content:
            return jsonify({
                'success': True, 
                'message': 'Web Analytics documentation already exists',
                'category_id': str(category_id),
                'content_id': str(existing_content['_id'])
            })
...
"""
        ...
        return jsonify({
            'success': True,
            'message': 'Web Analytics documentation created successfully',
            'category_id': str(category_id),
            'content_id': str(result.inserted_id)
        })
        ...
        return jsonify({
            'success': False,
            'message': f'Error seeding Web Analytics documentation: {str(e)}'
        }), 500
